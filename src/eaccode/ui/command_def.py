...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List file checkpoints; restore one with an index",
               "Session", args_hint="[N]"),
    CommandDef("clear", "Clear conversation history (confirm: --yes)", "Session"),
    CommandDef("tips", "Show a random usage tip (J.31)", "Session"),
    CommandDef("focus", "Refocus the input field (J.34)", "Session"),
    CommandDef("debug", "Stream diagnostics (J.2)", "Session"),
    ...
    CommandDef("status", "Show session, model, tokens, ctx%, workdir",
               "Session"),
    # ── Configuration ────────────────────────────────────────────────────
    CommandDef("mode", "Switch permission mode "
                       "(default|acceptEdits|plan|safeAuto|bypassPermissions)",
               "Configuration", args_hint="<name>",
               subcommands=("default", "acceptEdits", "plan", "safeAuto",
                            "bypassPermissions"),
               picker=True),
    CommandDef("pause", "Pause the session — no tool calls until /resume",
               "Configuration"),
    CommandDef("resume", "Resume a paused session", "Configuration"),
    CommandDef("allow", "Permanently allow a tool/pattern (allowlist)",
               "Configuration", args_hint="<tool> [pattern]"),
    CommandDef("disallow", "Remove a tool/pattern from the allowlist",
               "Configuration", args_hint="<tool> [pattern]"),
    CommandDef("approve", "Approve a pending permission ask by id (B.4)",
               "Configuration", args_hint="<id>"),
    CommandDef("deny", "Deny a pending permission ask by id (B.4)",
               "Configuration", args_hint="<id>"),
